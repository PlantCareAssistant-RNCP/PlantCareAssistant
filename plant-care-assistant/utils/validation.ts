...
export function validateEvent(body: EventInput): ValidationError | ValidEvent {
  ...
  try {
    ...
    return {
      ...
      plantId: typeof body.plantId === 'number' ? body.plantId : 
               typeof body.plantId === 'string' ? parseInt(body.plantId) : undefined
    };
  } catch (error) {
...
export function validatePlant(body: PlantInput): ValidationError | ValidPlant {
  // Check required fields
  const requiredError = validateRequiredFields(body, ["plant_name", "plant_type_id"]);
  if (requiredError) return requiredError;
...
export function validateUser(body: UserInput): ValidationError | ValidUser {
  const requiredError = validateRequiredFields(body, ["username", "email", "password"]);
  if (requiredError) return requiredError;
...
export function validatePost(body: PostInput): ValidationError | ValidPost {
  const requiredError = validateRequiredFields(body, ["title", "content", "plant_id"]);
  if (requiredError) return requiredError;
...
// Comment validator
export function validateComment(body: CommentInput): ValidationError | ValidComment {
...
export function isValidationError(result: ValidationError | unknown): result is ValidationError {
  return (
