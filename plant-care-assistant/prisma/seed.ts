...
async function seed() {
  try {
    ...
    let user1 = await prisma.userProfile.findUnique({ where: { username: "johndoe" } });
    let user2 = await prisma.userProfile.findUnique({ where: { username: "janedoe" } });
    let user3 = await prisma.userProfile.findUnique({ where: { username: "alice" } });
    let user4 = await prisma.userProfile.findUnique({ where: { username: "bob" } });
    ...
    const existingTypeNames = existingPlantTypes.map(pt => pt.plant_type_name.toLowerCase());

    const newPlantTypes = [
      "Succulent", "Flower", "Tree", "Aromatic Herb", "Cactus", 
      "Fern", "Vine", "Houseplant", "Vegetable", "Fruit Tree", 
      "Grass", "Moss", "Air Plant", "Aquatic Plant", "Bonsai"
    ...
    if (newPlantTypes.length > 0) {
      ...
      logger.info(`Added ${newPlantTypes.length} new plant types: ${newPlantTypes.join(', ')}`);
    } else {
      logger.info("All plant types already exist");
    }
    ...
    if (existingPlants.length === 0) {
      ...
      const succulentType = await prisma.plantType.findFirst({ where: { plant_type_name: "Succulent" } });
      const flowerType = await prisma.plantType.findFirst({ where: { plant_type_name: "Flower" } });
      const treeType = await prisma.plantType.findFirst({ where: { plant_type_name: "Tree" } });
      const herbType = await prisma.plantType.findFirst({ where: { plant_type_name: "Aromatic Herb" } });
      ...
      logger.info("Sample plants created");
    } else {
      logger.info("Plants already exist, skipping plant creation");
    }
    ...
    if (existingEvents.length === 0) {
      ...
      logger.info("Events already exist, skipping event creation");
    }
